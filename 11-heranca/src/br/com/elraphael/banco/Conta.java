...
public class Conta {
  ...
  public void creditarRendimentos(double percentualJuros) {
    if (this.getTipo() == INVESTIMENTO || this.getTipo() == ESPECIAL) {
      double valorRendimentos = this.getSaldo() * percentualJuros / 100;
      this.saldo += valorRendimentos;
      this.depositar(valorRendimentos);
    } else {
      throw new RuntimeException("Não podemos creditar rendimentos nesse tipo de conta");
    }
  }
  ...
  // apenas conta especial
  public void debitarTarifaMensal() {
    if (this.getTipo() == ESPECIAL) {
      sacar(this.getTarifaMensal());
    } else {
      throw new RuntimeException("Não é possível debitar tarifa mensal nesse tipo de conta");
    }
  }
}